...
void principale(SDL_Surface *ecran)
{
      ...
      SDL_Surface *new,*replay;
      replay = IMG_Load("replay.png");
      new = IMG_Load("new.png");//pour le boutons replay
      SDL_Rect preplay;
      preplay.x = 480;
      preplay.y = 0; 
      SDL_BlitSurface(replay, NULL,ecran,&preplay);
      time_t t;
      ...
      SDL_BlitSurface(replay, NULL,ecran,&preplay); // afichage du boutons replay
        ...
        if(event.type == SDL_QUIT)//si on veut sortir du programme 
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN ) // si on utulise la click de la souris 
            {
                  ...
                  if(event.button.button == SDL_BUTTON_LEFT)// click gauche du souris 
                  {
                        ...
                        if (event.button.x >=480 && event.button.x<=520 && event.button.y>=0 && event.button.y<=60)
                        {
                              move = NULL;
                              for (int j = 0; j <=9 ; ++j)
                              {
...
/*
  ...
  //fermeture du fontion principale 
}
